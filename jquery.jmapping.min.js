...
return $(this).metadata(settings.metadata_options);});};var getBounds=function(doUpdate){var newBounds,swPoint,nePoint;var places_data=getPlacesData(doUpdate);if(places_data.length){nePoint=swPoint=places_data[0].point;}else{nePoint=swPoint=settings.default_point;}
newBounds=new GLatLngBounds($.jMapping.makeGLatLng(swPoint),$.jMapping.makeGLatLng(nePoint));for(var i=1,len=places_data.length;i<len;i++){newBounds.extend($.jMapping.makeGLatLng(places_data[i].point));}
